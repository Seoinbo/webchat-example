((function ( $ ) {
  ...
  $.widget('aerolab.blockrain', {
    ...
    _SetupShapeFactory: function(){
      ...
      function Shape(game, orientations, symmetrical, blockType) {
        ...
        $.extend(this, {
          ...
          dropNow: function() {
            for (var i = game._BLOCK_HEIGHT;  i >= 0; i--) {
              if (!game._checkCollisions(this.x, this.y + i, this.getBlocks())) {
                this.y += i;
                // Reset the drop count, as we dropped the block sooner
                game._board.dropCount = -1;
                // game._board.animate();
      ...
      this._filled = {
        ...
        shift: function(count = 1) {
          for (var i = 0; i < game._BLOCK_WIDTH * game._BLOCK_HEIGHT; i++) {
            ...
            if (this.data[i] && this.data[i].blockIndex !== null) {
              this.data[dest] = {
                ...
                blockVariation: this.data[i].blockVariation, 
                blockIndex: this.data[i].blockIndex, 
                blockOrientation: this.data[i].blockOrientation
              };
              delete this.data[i];
            }
          }
          ...
          for (var p = 1; p <= count; p++) {
            for (var q = 0; q < game._BLOCK_WIDTH; q++) {
              if (q == hole) {
                continue;
              }
              this.data[this.asIndex(q, game._BLOCK_HEIGHT - p)] = {
                blockType: "line", 
                blockVariation: null, 
                blockIndex: 0, 
                blockOrientation: 0
    ...
    _setupControls: function(enable) {
      ...
      var shiftUpLine = function(count) {
        console.log("shift up!");
        console.log(game._filled.data);
        game._filled.shift(2);
      }
      ...
      var dropNow = function(start) {
        if( ! start ) { game._board.holding.drop = null; return; }
        if( ! game._board.holding.drop ) {
          game._board.cur.dropNow(); 
          game._board.holding.drop = Date.now();
        }
      }
      ...
      var handleKeyDown = function(evt) {
        ...
        switch(evt.keyCode) {
          ...
          case 32: /*space*/  dropNow(true); break;
